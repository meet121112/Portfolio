...
export function Navbar() {
  ...
  return (
    <header 
      className={cn(
        "fixed top-0 left-0 right-0 z-50 transition-all duration-300 px-4 md:px-8 flex items-center w-full fade-in",
        isScrolled 
      ...
      <div className="max-w-[1400px] mx-auto w-full flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center">
            <button className="text-sm font-semibold text-white/90 hover:text-white transition-opacity px-4 py-2">
              Sign In
            </button>
          </div>
          ...
          <div className="md:hidden">
            <Sheet>
              ...
              <SheetContent side="right" className="bg-[#0a0a0a]/95 backdrop-blur-2xl border-white/10 text-white w-[300px] p-8">
                <SheetTitle className="sr-only">Navigation Menu</SheetTitle>
                <div className="flex flex-col gap-8 mt-12">
                  <div className="text-xs font-bold tracking-widest text-primary uppercase opacity-60 mb-2">Navigation</div>
                  {navItems.map((item) => {
                    const isActive = pathname === item.href;
                    return (
                      <Link
